urun = [
    ...
    // 1
    let fiyat = urun.filter(product => product.unitPrice >= 30 && product.unitPrice <= 100);
    fiyatSayisi = fiyat.length;
    console.log(fiyatSayisi);

    // 2
  let aHarfi = urun.filter(product => product.name.toLowerCase().startsWith('a'));
console.log(aHarfi);
 
    
// 3

let productId = urun.find(product => product.id === 10);

    console.log(productId.name);

// 4

let notDiscountined = urun.filter(product => !product.discontinued);
let countnotDiscountined = notDiscountined.length;
console.log(countnotDiscountined);

// 5

let maxPrice = urun.reduce((maxProduct, currentProduct) => {
    return currentProduct.unitPrice > maxProduct.unitPrice ? currentProduct : maxProduct;
}, urun[0]);
console.log(maxPrice.name);

// 6

let minPrice = urun.reduce((minProduct, currentProduct) => {
    return currentProduct.unitPrice <  minProduct.unitPrice ? currentProduct : minProduct;
}, urun[0]);
console.log(minPrice.name);
